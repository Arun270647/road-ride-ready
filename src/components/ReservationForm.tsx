...
export const ReservationForm = ({ defaultCar }: ReservationFormProps) => {
  ...
  const [estimatedDropoffTime, setEstimatedDropoffTime] = useState<Date | null>(
    null
  );
  ...
  const handleEstimate = () => {
    if (
      ...
      !dropoffLocation?.geometry?.location ||
      !pickupDate ||
      !pickupTime
    )
      return;
      ...
      (result, status) => {
        if (status === google.maps.DirectionsStatus.OK && result) {
          ...
          if (leg?.duration && leg?.distance) {
            ...
            const dropoffDateTime = addSeconds(
              pickupDateTime,
              leg.duration.value
            );
            ...
            setEstimatedDistance("Not available");
          }
        } else {
          console.error(`Error fetching directions: ${status}`);
          setEstimatedDuration("Could not calculate");
          setEstimatedDistance("Could not calculate");
        }
        setIsLoading(false);
      }
    );
            ...
            <DropdownMenuTrigger asChild>
              ...
              >
                <span
                  className={cn(
                    "text-2x1 font-bold",
                    !selectedCar && "font-normal text-muted-foreground"
                  )}
                >
                  {selectedCar || "Select Your Car type"}
                </span>
          ...
          {estimatedDuration && (
            <div className="text-center p-3 bg-muted rounded-md space-y-1">
              <div className="flex justify-around font-semibold">
                <p>
                  Time: <span className="text-primary">{estimatedDuration}</span>
                </p>
                {estimatedDistance && (
                  <p>
                    Distance:{" "}
                    <span className="text-primary">{estimatedDistance}</span>
                  </p>
                )}
              </div>
